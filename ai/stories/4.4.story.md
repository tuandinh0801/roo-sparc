...
- **Unit Tests ([`tests/unit/commands/manageListModes.test.ts`](tests/unit/commands/manageListModes.test.ts:0), [`tests/unit/commands/manageListCategories.test.ts`](tests/unit/commands/manageListCategories.test.ts:0)):**
    - `manageListModes.ts` ([`src/commands/manageListModes.ts`](src/commands/manageListModes.ts:0)) / `manageListCategories.ts` ([`src/commands/manageListCategories.ts`](src/commands/manageListCategories.ts:0)) command handlers:
        - Mock `DefinitionLoader` ([`src/core/DefinitionLoader.ts`](src/core/DefinitionLoader.ts:0)) to return various datasets:
            - Only system definitions.
            - Only custom definitions.
            - A mix of system and custom definitions, including some with conflicting slugs.
            - Empty sets (no custom definitions, or even no system definitions if that's a possible state to test).
        - Verify that the command handler correctly processes these datasets based on the `--source` flag (`custom`, `system`, `all`, default).
        - Verify that the data passed to the (mocked) `UiManager.displayTable` method is correctly formatted (correct headers, rows containing slug, name, description, and accurate source tagging like 'system', 'custom', 'custom (overrides system)').
        - Test that the "no definitions found" message is correctly triggered and passed to `UiManager` when appropriate.
    - `DefinitionLoader` ([`src/core/DefinitionLoader.ts`](src/core/DefinitionLoader.ts:0)) (if new methods for filtering/tagging sources are added):
        - Unit test these new helper methods in isolation to ensure they correctly filter and tag definitions based on source.
- **End-to-End (E2E) Tests ([`tests/e2e/manage-commands.e2e.test.ts`](tests/e2e/manage-commands.e2e.test.ts:0) or similar):**
    - Test `roo-init manage list modes` and `roo-init manage list categories` commands by executing the CLI (via `cliTestRunner.ts`) with different `--source` flag values (`custom`, `system`, `all`, and no flag for default).
    - Set up various scenarios for `user-definitions.json` in a temporary user-global configuration directory (using `memfs`):
        - No `user-definitions.json` file.
        - Empty `customModes` / `customCategories` arrays.
        - `user-definitions.json` with only custom definitions (no slug conflicts with system).
        - `user-definitions.json` with custom definitions that override system definitions (conflicting slugs).
    - Verify the orchestration of `DefinitionLoader` and `UiManager` by the list command handlers.
    - Assertions:
        - Check the console output (captured by `cliTestRunner.ts`) for the correctly formatted table (or "not found" message).
        - Verify that the content of the table (slugs, names, descriptions, source tags) matches the expected data based on the system definitions and the prepared `user-definitions.json`.
        - Ensure correct precedence and source tagging for the `--source=all` option.
- **Manual/CLI Verification:**
    - Manually create/edit `~/.config/roo-init/user-definitions.json` with various custom modes and categories, including some that override system definitions and some that are unique.
    - Run `roo-init manage list modes` and `roo-init manage list categories` with all `--source` options (`custom`, `system`, `all`) and without the flag (to test default).
    - Verify that the output table accurately reflects the system and custom definitions, including correct source tagging and precedence.
    - Check table formatting, column alignment, and clarity of information.
    - Test the "no [custom] modes/categories found" messages by temporarily emptying or removing `user-definitions.json`.
